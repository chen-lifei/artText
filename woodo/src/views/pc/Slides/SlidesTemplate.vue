<template>
    <div class="slides_container" :class="{'skeleton-wrapper': !renderready}" @click="toggleThumbNail()">
        <div class="slides_center">
            ...
            <div class="slide_footer" v-show="!fullScreen">
                <div class="footer_left"><img :src="author_info.headUrl"/><span class="name">{{ author_info.name }}</span><span>创建时间：{{ template_info.createTime }}</span><span>模板ID：{{ template_info.id }}</span></div>
                <div class="footer_middle"><span class="doc_title">{{ template_info.name }}</span></div>
                <div class="footer_right">
                    ...
                    <a class="collect_btn" href="javascript:void(0);" :class="{collected:is_favorite}" @click="document_favorite">收藏</a>
                    <a class="share_btn" href="javascript:void(0);"  @click="open_share">分享</a>
                </div>
            </div>
        </div>
        <!-- 推荐列表 全屏需隐藏 -->
    ...
    metaInfo() {
        return {
            ...
            meta: [
                ...
                {
                    property: 'og:url',
                    content: `https://www.woodo.cn/template/slides/${this.$store.state.metaInfo.query}/`
                },
            ],
        };
    },
    ...
    methods: {
        ...
        get_document_info(id, callback) {
            ...
            that.$axios.get(`/api/template/detail/${id}/`).then(res => {
                ...
                that.author_info = res_data.data.author;
                that.document_pages = document_pages;
                that.is_favorite = res_data.data.collect;
                // 模板信息
                that.template_info = res_data.data.template;
                that.template_info.createTime = utils.timeStampDetail(that.template_info.createDate / 1000); 
                // 生成分享二维码
        ...
        create_share_qrcode() {
            let that = this;
            let url = `${location.origin}/mobile/template/slides/${that.template_info.id}`;
