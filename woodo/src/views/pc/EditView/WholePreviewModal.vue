<template>
    <transition name="modal-fade">
        <div class="whole-preview-modal" v-show="show_modal" @click.self="close()">
            <div class="centent" ref="centent" @mousewheel="wheel($event)">
                <!-- 主要显示内容 -->
                <div class="main-content">
                    ...
                    <div class="insert-btn" @click="insert" v-if="show_insert">
                        <base-icon class="icontianjia1"></base-icon>
                        插入使用
                        <div class="vip-icon" v-if="current_page_info.isVip">
...
export default {
    ...
    methods: {
        // 显示模态框
        show({ id, type, is_suit }) {
            this.init();
            // 如果是全套，则预览不显示插入
            if (is_suit) {
                this.show_insert = false;
            }
            this.getTemplateDetail(id, type);
        },
        group_show(data, index) {
            this.init();
            ...
            this.current_page_info = doc_pages[i];
            // 计算svg尺寸
            this.$nextTick(() => {
                this.computeSvgSize();
        ...
        init() {
            ...
            this.show_insert = true;
        },
        getTemplateDetail(id, type) {
            let url = '';
            let detail_type = type || 'template';
            switch (detail_type) {
                // 模板
                case 'template':
            ...
            return new Promise((resolve, reject) => {
                this.$axios.get(url + id).then(res => {
                    ...
                    if (code === "2") {
                        ...
                        this.$toast(data.data.content, 800);
                        reject(res);
                    }
                }).catch(reject);
            });
        ...
        // 插入
        insert() {
            let info = this.type === 'design' ? this.template_info : this.current_page_info
