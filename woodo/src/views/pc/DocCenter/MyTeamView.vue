				...
				<div class="create_team_contain" :class="{invite:team_info}">
					...
					<div class="body">
						...
						<div class="search_member_result">
							...
							<!--列表（已邀请）-->
                            <template v-if="team_member_invitation.added_member.length > 0 && team_member_invitation.search_keyword === ''">
							...
							<!--列表（搜索）-->
                            <template v-if="team_member_invitation.search_result.length > 0 && team_member_invitation.search_keyword !== ''">
                                <div class="partner_item"
...
import DocListMain from '@/views/pc/DocCenter/components/DocListMain.vue';
Vue.use(QRCode);
export default {
    name: 'docTeam',
    components:{
    ...
    methods: {
        ...
        timestamp_to_time: function(arr) {
            for(let i = 0; i < arr.length; i++) {
                if(arr[i].modifyDate){
                    arr[i].time = utils.timeStampDetail(arr[i].modifyDate / 1000);
                }else if(arr[i].createDate){
                    arr[i].time = utils.timeStampDetail(arr[i].createDate / 1000);
                }
            ...
            if (type === 'link') {
                ...
                if (!that.team_invite_url) {
                    that.team_invite_key('link',function(data){
                        ...
                        url += "&key=" +  data.data;
                        url += "&ownerName=" +  utils.getCookies('woodo_memberNickName');
