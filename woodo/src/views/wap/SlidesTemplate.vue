<template>
    <div class="slides_contain">
        ...
        <div class="slides_wrapper" v-else>
            ...
            <div class="toolbar vertical" :class="{'top': share_referrer, 'show': show_tool}" v-else>
                ...
                <div class="opeartion_bar" v-if="share_referrer">
                    ...
                    <!-- 下载 -->
                    <div class="opeartion_btn opeartion_share_btn" @click="template_download">
                ...
                <div class="opeartion_bar" v-else>
                    ...
                    <!-- 下载 -->
                    <div class="opeartion_btn opeartion_download_btn" @click="template_download">
                    ...
                    <div class="opeartes_btn">
                        <!-- 分享 -->
                        <div class="opeartion_btn share_btn" @click="share_document">
                            <img src="../../assets/wap/images/display/share_w.png" alt=""/>
                        </div>
                        <!-- 收藏 -->
                        <div class="opeartion_btn opeartion_favorite_btn" @click="favorite_document">
        ...
        <!-- 下载弹窗 -->
        <export_modal ref="export_modal" :doc_name="template_info.name"></export_modal>
    </div>
</template>
    ...
    metaInfo() {
        return {
            ...
            meta: [
                ...
                {
                    property: 'og:url',
                    content: `https://www.woodo.cn/mobile/template/slides/${this.$store.state.metaInfo.query}/`
                },
            ],
        }
    },
    asyncData({store,host,route}) {
        return store.dispatch('setMeta',{url:`${host}/api/common/template/url_info/`, params:{id:route.params.id}});
    },
    mounted() {
        let that = this;
    ...
    methods: {
        ...
        get_document_info(id) {
            ...
            that.$axios.get(`/api/template/detail/${id}/`).then(res => {
                ...
                // 模板信息
                that.template_info = res_data.data.template;
                // 获取文档作者信息
                that.get_document_author_info(document_info.id);
                // 微信端 调用微信分享接口
                if (utils.deviceENV().wechat) {
                    let title = `${that.template_info.name}-吾道`,
        ...
        // 获取相关作品
        get_relevant_data(templateId) {
            let that = this;
            let cid = that.author_info.categoryId;
            if (!cid) {
                return;
            }
            that.$axios.get('/api/template/related_template/', {
                params: {
                    'cId': cid,
                    'count': 4,
        ...
        relevant_document_open(item) {
            ...
            window.location.href = `/mobile/template/slides/${item.id}/`;
        },
        ...
        share_document() {
            ...
            if (that.is_miniprogram) {
                that.wx_miniprogram.navigateTo({
                    'url': `/pages/share/share?type=template&id=${that.template_info.id}`,
                });
            } else {
                window.location.href = `/mobile/doc_share/?type=template&id=${that.template_info.id}`;
            }
        },
        // 收藏文档
        favorite_document() {
        ...
        template_download() {
            let that = this,
                id = that.template_info.id;
            if(that.template_info.isVip && !that.user.memberVipExpire){
                that.$modal({
                    modalClass: `template-vip-modal mobile`,
                    modalTitle: '下载提示',
                    modalContent: `<p>抱歉！会员专享模板，请升级会员后继续下载</p>`,
                    buttonCancelTxt: '立即升级',
                    buttonSubmitTxt: '取消',
                    showClose:false,
                ...
                if (that.is_miniprogram) {
                    that.wx_miniprogram.navigateTo({
                        'url': `/pages/download/download?id=${id}&title=${encodeURI(that.template_info.name)}&type=template`,
                    });
                }else{
                    that.$refs.export_modal.show(id, that.template_info.name, 'template');
                }
            }
        },
    }
