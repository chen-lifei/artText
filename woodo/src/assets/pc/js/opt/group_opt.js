...
let group_opt = {
	...
	ele_paste: function(obj, deviation){
		// 错误状态判断
		...
		let ele_list = obj.list,
			...
			ele_html;
		// 存在组合id情况
		if (o_group) ele_list.forEach(function(){n_group.push(base_opt.fn.uuid())});
		// 遍历元素对象列表
		ele_list.forEach(function(item){
			// 修改元素id
	...
	// 设置元素超链接
	ele_link_add: function(ele,link){
		// 错误状态判断
		if (!ele || ele.length !== 1) {
			console.error('params type error');
			return false;
