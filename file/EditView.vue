...
<template xmlns:v-clipboard="http://www.w3.org/1999/xhtml">
    <div class="edit_content" @mouseover="init_reload_interval" :class="edit_mode">
        ...
        <div class="edit_body_wrapper" :class="{'left-hide': !show_left_modal,}">
            ...
            <!-- 编辑器右侧栏 -->
            <div class="edit_right" :class="{hide: right_panel_nav.length === 0}">
                ...
                <!-- 格式面板 -->
                <format-panel ref="FormatPanel" :edit_mode="edit_mode" :element_type="element_type" :is_group="is_group" :element_message="element_message" :hide_right="current_right_panel !== 'format'"></format-panel>
            </div>
            <!-- 全局设置弹框 -->
...
import collaborateSocket from '@/components/Collaborate/CollaborateSocket.js';
...
export default {
    ...
    data() {
        return {
            ...
            **/
            ...
            all_collaborators_list: [],                   // 全部协作者列表
            document_authority_info: {},                  // 文档协作信息
            online_list: [],
            user_disable_edit: false,
            user_authority: collaborate.getMap('owner'),
            collaborate_socket: collaborateSocket,
    ...
    mounted() {
        ...
        that.render_watermark = utils.debounce(that.render_watermark, 300);   // 水印绘制节流处理
        // 设置富文本功能使用css
    ...
    methods: {
        ...
        keydown_listener: function (event) {
                ...
                switch (code) {
                    ...
                    case 75:
                        if (has_focus) {
                            ...
                            let linkArr = ['text', 'shape', 'img', 'chart', 'table'];
                            if ($checked.length === 1 && linkArr.indexOf(this.element_type) > -1) {
                                that.checkLinkExist();
                                that.addEleLink();
                            } else {
            ...
            if (ctrl && shift && !alt) {
                ...
                switch (code) {
                    ...
                    // ctrl + shift + G 取消组合
                    case 71:
                        if (get_checked_ele.group && !that.element_message.icon_park) {
                            that.set_ele_group();
                            that.ele_cancel_checked();
                        }
                        e.preventDefault();
            ...
            try {
                ...
                ws_client({
                    success: function (method) {
                        ...
                        // 文档协作
                        that.collaborate_socket.int(that.ws_client_method, 'edit');
                    },
                    error: function (msg) {
                        console.error(msg);
                    }
        ...
        ws_doc_authority_rcvd: function (data) {
            try {
                ...
                if (data.partner) {
                    ...
                    let { id: new_role } = collaborate.getMap(new_partner.collaborateRoleType);
                    let { id: old_role } = user_authority;
                    // 判断当前用户权限变更
                    if (new_role !== old_role) {
                        switch (true) {
                            ...
                            case new_role < old_role:
                                ...
                                that.user_authority = collaborate.getMap(new_partner.collaborateRoleType);
                                break;
                        }
                    }
                }
                // 更新当前用户权限
                that.get_all_collaborators();
            } catch (e) {
                console.error(e);
            }
        },
        ...
        init_document_data_error: function (res) {
            ...
            if (content === 'documentAccessDenied' || content === '没有文档的相关权限') {
                window.location.href = `/document/slides/${data.url}/`;
            } else {
        ...
        start_document_watcher: function () {
            ...
            let document_data_mapping = function () {
                ...
                //3、权限
                if (that.user_authority.type !== 'owner' && that.page_info.isLock) {
                    that.user_disable_edit = true;
                }
            };
        ...
        start_document_observer: function () {
            ...
            let observer_filter = function (mutations) {
                ...
                for (let i of mutations) {
                    ...
                    if (that.searchReplace) continue;
                    pass = false;
                    break;
                }
                if (!pass) {
        ...
        render_watermark: function() {
            let info = this.document_authority_info;
            let owner = info.documentCollaborates.find(i => i.collaborateRoleType === 'owner');
            this.$common.createWatermark({ owner, setting: info.setting, wrapper: document.querySelector('.edit_body') });
        },
        /*文档相关方法 end*/
        ...
        set_ele_checked: function (ele, obj = {}) {
            ...
            if (type === 'shape') {
                // 形状元素编辑
                edit_shape_point(ele, (fillet, fillet_max,fillet_together_set) => {
                    that.element_message.rect_fillet = fillet;
                    that.element_message.rect_fillet_max = fillet_max;
                    that.element_message.fillet_together_set = fillet_together_set;
                });
            } else if (type === 'line') {
                // 线条元素编辑
                edit_line_point(ele);
        ...
        ele_copy: function (e, needPaste) {
            let that = this,
                ...
                text_range = '';
            // 重复触发拦截
            if (e && (e.target.nodeName === 'INPUT' || e.target.type === 'textarea' || is_fakeEditor)) return false;
        ...
        ele_cancel_checked: function () {
            ...
            let $checked = $('.page_contain .ele_item.checked');
            // 去除视频
            if (!utils.checkIsFull() || that.show_slides_preview) {
        ...
        checkLinkExist() {
            ...
            if (this.element_type === 'text' || this.element_type === 'table') {
                ...
                if (isRange) {
                    ...
                    $selector.each((index, dom) => {
                        ...
                        if (dom.nodeName === 'A') {
                            ...
                            aEle = $(dom).parents('a').length ? $(dom).parents('a') : $(dom).children('a');
                        }
                        if (aEle.attr('href')) {
                            this.ele_link_info = {
                                text: window.getSelection ? selection.toString() : selection,
                                link: aEle.attr('href')
                            }
                            return;
                        }
                    ...
                    let aEle = $ele.find('a');
                    aEle.each((aIndex, aDom) => {
                        aDom = $(aDom);
                        if (aDom.attr('href')) {
                            this.ele_link_info = {
                                text: $ele.find('.text_content .show_text')[0].innerText,
                                link: aDom.attr('href')
                            }
                            return;
                        }
                ...
                if ($rotate.attr('title')) {
                    this.ele_link_info = {
                        link: $rotate.attr('title')
                    }
                }
            }
        },
        ...
        addEleLink(type) {
            ...
            if (this.element_type === 'text' || this.element_type === 'table') {
                let selection = window.getSelection ? window.getSelection() : document.selection.createRange().text;
                let isRange = !!($ele.find('[contenteditable="true"]').length && selection && selection.type === 'Range' && !selection.isCollapsed);
                if (this.element_type === 'table' && !isRange) return this.$toast('请框选中表格内文本后操作', 2000);
                $ele = $ele.find('.show_text');
                if (isRange) {
                    editor_opt.save_text_selection($ele);
                    linkParams['text'] = window.getSelection ? selection.toString() : selection;
                } else {
                    editor_opt.set_selection($ele);
                    editor_opt.save_text_selection($ele);
                    linkParams['text'] = window.getSelection ? selection.toString() : selection;
                }
            } else {
                $ele = $ele.find('.ele_rotate');
            }

            setTimeout(() => {
                if (type === 'draw') {
                    linkParams['type'] = 'draw';
                    $ele = $('.page_contain .ele_item.checked');
                    if ($ele.find('.text_content .show_text')) $ele = $ele.find('.text_content .show_text');
                }
                this.openLinkPanel({
                    isEdit: true,                 // 右键菜单添加 / 编辑 超链接时打开面板
                    target: $ele[0],
                    linkParams: this.ele_link_info || linkParams
                });
            }, 10);
        },
        // 打开超链接弹框
        openLinkPanel(data) {
            let { isEdit, setCheck, target, linkParams } = data;
            linkParams = Object.assign({
                type: this.element_type, text: '', link: '', page: ''
            }, linkParams);
            linkPanel.create({
                isEdit: isEdit,
                target: target,
                ...
                changePage: (data) => {
                    ...
                    let targetPage = this.document_page_list[targetIndex].uuid;
                    let currentPage = this.$refs.sort_page_modal.current_page_uuid;
                    let currentIndex = this.document_page_list.findIndex(page => page.uuid === currentPage);
                    if (currentIndex == targetIndex) return;
                    this.$refs.sort_page_modal.current_page_uuid = targetPage;
                    page_opt.change(this, targetIndex, targetIndex - currentIndex ? 'next' : 'previous');
                },
                remove: () => {
                    this.removeEleLink(linkParams.type);
                },
                destroy: () => {
                    this.clearTextRange();
        ...
        setEleLink(data) {
            ...
            let option = opt_factory.init_opt('group');
            let textOption = opt_factory.init_opt('text');
            let tableOption = opt_factory.init_opt('table');
            if (this.current_link_dom) {
                option.ele_link_change($(this.current_link_dom), data, () => {
                    // 设置成功后，更新虚线框
                    let $check = $('.ele_item.checked');
                    if (data.type === 'text') {
                        $check.each((i, dom) => {
                            textOption.set_ele_sync_content($(dom));
                        });
                    } else if (data.type === 'table') {
                        tableOption.table_edit_listener($check)
                    }
                    operate_opt.reset_rect($check);
                });
                return;
            };
            // 文字工具面板，插入一个超链接
            if (data.type === 'draw' && textOption.using_data.text_obj && textOption.using_data.text_obj.type === 'link') {
                let textStyle = textOption.using_data.text_obj.style;
                let aEle = `<span><a href=${link} style='font-size: ${textStyle.fontSize * 2}px; color: ${textStyle.color}; background-color: ${textStyle.backgroundColor}; font-weight: ${textStyle.fontWeight}; font-family: ${textStyle.fontFamily}; font-style: ${textStyle.fontStyle}; text-decoration: ${textStyle.underline} ${textStyle.lineThrough};'>${text}</a></span>`;
                $ele.find('.show_text.example_text').append(aEle);
                return;
            }

            data['range'] = this.linkRange;
            option.ele_link_add($ele, data, () => {
                ...
                if (data.type === 'text') {
                    ...
                    tableOption.table_edit_listener($ele)
                }
                operate_opt.reset_rect($ele);
            });
        },
        ...
        removeEleLink(type) {
            let option = opt_factory.init_opt('group');
            if (this.current_link_dom) {
                option.ele_link_remove($(this.current_link_dom), type);
            } else {
                option.ele_link_remove($('.page_contain .ele_item.checked'));
            }
        },
        /* 超链接相关 end*/
            ...
            try {
                switch (type) {
                    ...
                    case 'clean':
                        ...
                        $text.attr('style', `font-size:32px; font-family:woodo-SourceHanSansCN-Light; color:${this.theme_checked.textColor};`).find('*').not('br').attr({'style': '', 'href': ''});
                        break;
                }
                // 重置文本框大小
         ...
         */
        // 元素插入前生成元素预览图、预插入绘制层
        element_preinsert_ready: function (element_list = [], set_group) {
            let that = this;
        ...
        /* 协作者相关 start*/
        get_all_collaborators: function () {
            const that = this;
            collaborate.authority(that.document_info.id).then(data => {
                let list = data.documentCollaborates;
                that.all_collaborators_list = list;
                data.setting = data.setting && data.setting  != 'null' ? utils.deepClone(JSON.parse(data.setting)) : {watermark: {}};
                that.document_authority_info = data;
                that.user_authority = collaborate.getMap(list.find(i => i.memberId === that.user.id).collaborateRoleType);
                that.render_watermark();
                that.$axios.get('/api/member/document/team_detail/' + that.document_info.id + '/').then(res => {
                    if (res.data.code == 2) {
                        list.forEach(item => {
                    ...
                    that.all_collaborators_list = list;
                    collaborateSocket.online.send([that.document_info.id.toString()], (online) => {
                        that.online_list = online[that.document_info.id] || [];
                    });
                })
            });
        },
        // 检查当前功能是否禁用
        check_option_available: function (availableRole = 'owner') {
            // 判断当前是否为模板进入
            if (this.$route.query.modalId) return false;
            // 判断当前是否存在文档
            if (this.document_info.id === null) return false;
            // 非此角色（availableRole）不可操作
            if (this.user_authority.type !== availableRole) return false;
            return true;
        },
        /*协作者相关 end*/
        ...
        page_event_initial: function () {
            ...
            operate_opt.bind_move_event({
                ...
                move: function (data, diff_x, diff_y) {
                    ...
                    if (get_element.type === 'group') {
                        get_element.element.each((i, ele) => {
                            let $ele = $(ele);
                            let p1 = option.get_transform($ele, 'translate');
                            let p2 = option.get_ele_transform($ele[0].style['transform'], 'translate') || [0,0];
                            group_ele_offset.x.push(Math.floor(p1[0] + p2[0]) + x_diff - ele_rect.x);
                            group_ele_offset.y.push(Math.floor(p1[1] + p2[1]) + y_diff - ele_rect.y);
                        });
                    }
                    // x轴方向自动贴合检索
                    let xfit_arr = [];
                    ...
                    if (slow && (xfit_arr.length || yfit_arr.length)) {
                        ...
                        if (get_element.type === 'group') {
                            ...
                            get_element.element.each((i, ele) => {
                                ...
                                let x = group_ele_offset.x[i] + point.x - p[0];
                                let y =  group_ele_offset.y[i] + point.y - p[1];
                                option.set_ele_transform($ele, 'translate', {x, y});
                            });
                        } else {
                            ...
                            let y = point.y - p[1];
                            option.set_ele_transform($ele, 'translate', {x, y});
                            rect_offset = [x * page_scale, y * page_scale];
                        }
                    } else {
                        // 元素正常移动
                        get_element.element.each((i, ele) => {
                            let $ele = $(ele);
                            let p = option.get_ele_transform($ele[0].style['transform'], 'translate') || [0,0];
                            let x = p[0] + (diff_x / page_scale);
                            let y = p[1] + (diff_y / page_scale);
                            option.set_ele_transform($ele, 'translate', {x, y});
                            if(get_element.element.length === 1){
                                rect_offset = [x * page_scale, y * page_scale]
                            }
                        });
                    }
                    ...
                    // 更新虚线框定位
                    operate_opt.reset_rect(get_element.element, { rect_offset });
                },
                ...
                up: function (data) {
                    ...
                    get_element.element.each((i, ele) => {
                        let $e = $(ele);
                        let p1 = option.get_transform($e, 'translate');
                        let p2 = option.get_ele_transform($e[0].style['transform'], 'translate') || [0,0];
                        let rx = p1[0] + p2[0];
                        let ry = p1[1] + p2[1];
                        option.set_translate($e, rx, ry);
                        option.set_ele_transform($e, 'translate', null);
                    });
            ...
            // 鼠标经过超链接
            $(document).on('mouseover', '.edit_body .edit_page .ele_item', (event) => {
                if (document.querySelector('.link-panel .link-edit') || document.querySelector('.right-menu-panel')) return;
                let target = $(event.target);
                let ele;
                let linkInfo;
                if (event.target.nodeName == 'A' || target.parents('a').length) {
                    ele = event.target.nodeName == 'A' ? target : target.parents('a');
                    if (ele[0] == this.current_link_dom) return;
                    if (!ele.attr('href')) return;
                    linkInfo = {
                        type: target.parents('.ele_item').attr('data-type'),
                        text: ele[0].text,
                        link: ele.attr('href')
                    }
                } else {
                    if (target.hasClass('ele_rotate')) {
                        ele = target;
                    } else {
                        ele = target.parents('.ele_rotate').length ? target.parents('.ele_rotate') : target.children('.ele_rotate');
                    }
                    if (!ele.attr('title')) return;
                    linkInfo = {
                        type: target.parents('.ele_item').attr('data-type'),
                        link: ele.attr('title')
                    }
                ...
                if (ele[0] == this.current_link_dom) return;
                this.current_link_dom = ele[0];
                this.openLinkPanel({
                    isEdit: false,
                    setCheck: linkInfo.type === 'text' || linkInfo.type === 'table',
                    target: ele[0],
                    linkParams: linkInfo
                });
            }).on('mouseout', '.edit_body .edit_page .ele_item', (event) => {
                if (document.querySelector('.link-panel .link-edit')) return;
                if (!$(event.toElement).parents('.link-panel').length) this.closeLinkPanel();
            });
            // 元素悬浮方法
