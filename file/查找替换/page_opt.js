...
let page_opt = {
    ...
    change : function(that, index, action, callback){
        let _this = this;
    ...
    //设置文档标题
    set_document_title : function(that, error){
    ...
    // 存储画布偏移尺寸
    save_page_offset: function(that){
        page_opt.page_size = {
            top_panel: $('.edit_head')[0].clientHeight,
            bottom_panel: $('.edit_footer')[0].clientHeight,
            left_panel: that.show_left_modal ? $('.edit_left').innerWidth() : 0,
            right_panel: that.right_panel_nav.length > 0 ? $('.edit_right').innerWidth() : 0,
            top_offset: 25,
            left_offset: 26,
            right_offset: 26,
        }
    ...
    set_page_scale: function(that){
        ...
        $body.css({
            "width": `calc(100% - ${size.left_panel + size.right_panel}px)`,
        });
        ...
        if (window_w - (size.left_panel + size.left_offset + size.right_panel) < page_w_scaled){
            ...
            $outer.css({
                'margin': `-${page_h_scaled / 2}px 0 0 -${page_w_scaled / 2 - 5}px`,
                'top': '50%',
                'left': '50%'
            });
            if (window_h - (size.top_panel + size.top_offset + size.bottom_panel) < page_h_scaled) {
                ...
                $outer.css({
                    'margin': `${size.top_offset}px 0 ${size.bottom_panel}px -${page_w_scaled / 2 + 15}px`,
                    'top': '0',
                });
            }
        }
    ...
    set_page_background: function($page, background){
        ...
        // 无背景色时默认白色
        if (!color_data || color_data === 'transparent' || (image_data && image_data.src.indexOf('png') < 0)) {
            color_data = "#ffffff";
        }
        // 获取背景样式
