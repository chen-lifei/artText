...
let image_opt = {
	...
	// 图片上传
	image_upload: function (file, name, isUploadFavorite = false, isCompress = true) {
		...
		form.append('isUploadFavorite', isUploadFavorite);
		return new Promise((resolve, reject) => {
			if (window.FileReader && file.type === 'image/svg+xml') {
				...
				this.image_compress(file).then(res => {
					if (res) form.set('file', res.file, name || res.name);
					resolve(true);
				});
			} else {
            	resolve(true);
	...
	/**
	 * 压缩图片分辨率
	 * @param {*} file 可以是图片对象，或者是图片地址
	 * @param {*} resolution 分辨率大小，默认2K
	 * @param {*} quality 压缩图片的质量，同 canvas.toBlob 的默认值，为0.92
	 * @returns promise 对象
	 */
	image_compress: function (file, resolution = 2560, quality = 0.92) {
		return new Promise(resolve => {
			try {
				let that = this;
				let compressImg = function(img, type, callback) {
					let image = new Image();
					image.src = img;
					// 解决图片地址跨域
					if (typeof img === 'string') image.setAttribute('crossOrigin', 'anonymous');
					image.onload = function() {
						let drawCanvas = function (imageWidth, imageHeight) {
						...
						if (image.naturalHeight <= resolution) {
							if (type === 'jpeg' && typeof file === 'object') {
								that.get_image_orientation(file).then(orientation => {
									...
									if (orientation && orientation != 1) {
										...
										aliyun_log.push(log);
										drawCanvas(image.naturalWidth, image.naturalHeight);
									} else {
										resolve();
									}
								});
							...
							let imageWidth = imageHeight * (image.naturalWidth / image.naturalHeight);
							imageWidth = Math.ceil(imageWidth);
							drawCanvas(imageWidth, imageHeight);
						}
					}
				}

				if (typeof file === 'object' && /(png|jpeg)$/.test(file.type)) {
					let size = file.size;
					let name = file.name;
					...
					reader.onload = function(e) {
						compressImg(e.target.result, type, function(blob) {
							// 如果图片格式是 image/png，生成的图片体积可能会变大，所以当体积变大时不使用压缩后的图片
					...
					if (!['png', 'jpg', 'jpeg'].includes(type)) return resolve();
					if (type === 'jpg') type = 'jpeg';
					compressImg(file, type, function(blob) {
						resolve(blob);
					});
				} else {
					resolve();
	...
	get_image_orientation: function(file) {
		return new Promise(resolve => {
            ...
            reader.onload = function(e) {
				...
				// 判断是否为 jpeg 类型的图片
                if ((dataView.getUint8(0) != 0xFF) || (dataView.getUint8(1) != 0xD8)) return resolve();
                let offset = 2,
	...
	// 添加图片至我的上传 -> 返回promise
	add_myupload: function(template,obj, name){
		return new Promise(function(resolve,reject){
