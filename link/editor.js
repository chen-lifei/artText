...
let editor_opt = {
	...
	set_link: function (ele, data) {
		...
		if(aDom.length) {
			...
			let noHasUnderline = true;
			switch (rangeNode.nodeName) {
				case 'A':
					linkElement = $(rangeNode);
					$(rangeNode).parents('span').each(function(index, domEle) {
						noHasUnderline = $(this).css('textDecoration') !== 'underline';
						return noHasUnderline;
					});
					break;
				case '#text':
					linkElement = $(rangeNode.parentNode);
					$(rangeNode).parents('span').each(function(index, domEle) {
						noHasUnderline = $(this).css('textDecoration') !== 'underline';
						return noHasUnderline;
					});
					break;
				case 'SPAN':
					linkElement = $(rangeNode).find('a');
					noHasUnderline= rangeNode.style.textDecoration !== 'underline';
					break;
			}
			// 如果原本没有下划线就添加下划线
			if(noHasUnderline) {
				cssOptions['text-decoration'] = 'underline';
			}
			linkElement.css(cssOptions);
		}
	},
	// 删除文本超链接
	remove_link: function (linkElement) {
		let link = linkElement.attr('href');
		let linkElements = linkElement.parents('.show_text').find(`a[href="${link}"]`);
		editor_opt.set_selection(linkElements);
		linkElements.attr('style', '');
		// 删除超链接
	...
	get_font_format: function(ele){
		...
		if (window.getSelection()) {
			...
			if (!select_node) {
				let text_node = $text[0];
				select_node = text_node.childNodes[0];
				while (select_node.nodeType !== 3) {
					select_node = select_node.childNodes[0];
                }
			}
			// 判断选中节点是否为span
